...
using FlowStack.TaskService.Middleware;
...
try
{
    ...
    builder.Services.AddSwaggerGen(options =>
    {
        ...
        {
            Title = "FlowStack Task Service",
            Version = "v1",
            Description = "Card/task management for FlowStack Kanban boards"
        });
    ...
    builder.Services.AddHealthChecks()
        .AddDbContextCheck<CardDbContext>("task-db");

    var app = builder.Build();

    //  Middleware Pipeline  
    // ORDER IS CRITICAL
    app.UseCorrelationId();           // 1. Assign CorrelationId first
    ...
    app.UseAuthentication();   // 4. Populate HttpContext.User from JWT
...
{
    Log.Fatal(ex, "FlowStack.TaskService failed to start.");
}
finally
{
    Log.CloseAndFlush();
