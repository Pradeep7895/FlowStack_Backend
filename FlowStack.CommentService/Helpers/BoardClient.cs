namespace FlowStack.CommentService.Helpers;
...
public class BoardClient
{
    ...
    public async Task<BoardAccessResult> GetBoardAccessAsync(Guid boardId, Guid userId)
    {
        try
        {
            ...
            {
                _logger.LogWarning("board-service returned {StatusCode} for board {BoardId} user {UserId}",
        ...
        {
            _logger.LogWarning(ex,"Board access check failed for board {BoardId} user {UserId}. Denying access to fail safe.",
                boardId, userId);
            return BoardAccessResult.Denied();
        }
    }
